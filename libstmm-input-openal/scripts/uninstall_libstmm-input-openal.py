...
# File:   uninstall_libstmm-input-openal.py

# Removes all files installed by the install_libstmm-input-openal.py script.
...
def main():
	import argparse
	oParser = argparse.ArgumentParser("Removes all files created by install_libstmm-input-openal.py")
	...
	if not oArgs.bNoUninstall:
		subprocess.check_call("{} rm -r -f             {}/include/stmm-input-openal".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f              {}/lib/libstmm-input-openal.so*".format(sSudo, sInstallDir), shell=True)
		subprocess.check_call("{} rm    -f              {}/lib/libstmm-input-openal.a".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f       {}/lib/pkgconfig/stmm-input-openal.pc".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm -r -f        {}/share/doc/libstmm-input-openal".format(sSudo, sInstallDir).split())
		if sInstallDir == "/usr":
			subprocess.check_call("{} rm  -f   /etc/stmm-input-dl/stmm-input-openal.dlp".format(sSudo).split())
		else:
			subprocess.check_call("{} rm  -f {}/etc/stmm-input-dl/stmm-input-openal.dlp".format(sSudo, sInstallDir).split())

	if not oArgs.bNoClean:
		os.chdir("..")
		subprocess.check_call("{} rm -r -f libstmm-input-openal/build".format(sSudo).split())
		subprocess.check_call("{} rm -r -f libstmm-input-openal/examples/wander/build".format(sSudo).split())
