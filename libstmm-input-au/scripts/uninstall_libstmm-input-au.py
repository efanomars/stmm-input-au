...
# File:   uninstall_libstmm-input-au.py

# Removes all files installed by the install_libstmm-input-au.py script.
...
def main():
	import argparse
	oParser = argparse.ArgumentParser("Removes all files created by install_libstmm-input-au.py")
	...
	if not oArgs.bNoUninstall:
		subprocess.check_call("{} rm -r -f             {}/include/stmm-input-au".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f              {}/lib/libstmm-input-au.so*".format(sSudo, sInstallDir), shell=True)
		subprocess.check_call("{} rm    -f              {}/lib/libstmm-input-au.a".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f       {}/lib/pkgconfig/stmm-input-au.pc".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm -r -f        {}/share/doc/libstmm-input-au".format(sSudo, sInstallDir).split())
		if sInstallDir == "/usr":
			subprocess.check_call("{} rm  -f   /etc/stmm-input-dl/stmm-input-au.dlp".format(sSudo).split())
		else:
			subprocess.check_call("{} rm  -f {}/etc/stmm-input-dl/stmm-input-au.dlp".format(sSudo, sInstallDir).split())

	if not oArgs.bNoClean:
		os.chdir("..")
		subprocess.check_call("{} rm -r -f libstmm-input-au/build".format(sSudo).split())
